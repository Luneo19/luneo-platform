...
export class AppBridgeService {
  ...
  async getSession(shop: string): Promise<AppBridgeSession | null> {
    try {
      ...
      // Récupérer depuis EcommerceIntegration (utilise shopDomain)
      // Note: Cette implémentation nécessite PrismaService injecté
      // Pour l'instant, utiliser ShopifyService qui peut accéder à la DB
      const shopData = await this.shopifyService.getShopData(shop);
      
      if (!shopData || !shopData.accessToken) {
        this.logger.warn(`Aucune session trouvée pour le shop: ${shop}`);
        return null;
      }
      
      return {
        id: shopData.sessionId || `session_${shop}_${Date.now()}`,
        shop,
        state: shopData.state || 'active',
        isOnline: shopData.isOnline || false,
        scope: shopData.scope || 'read_products,write_products',
        expires: shopData.expiresAt || new Date(Date.now() + 24 * 60 * 60 * 1000),
        accessToken: shopData.accessToken,
        onlineAccessInfo: shopData.onlineAccessInfo,
      };
    } catch (error) {
      this.logger.error('Erreur lors de la récupération de la session:', error);
      // Fallback: retourner null au lieu de throw pour permettre la création d'une nouvelle session
      return null;
    }
  }
  ...
  async updateSession(shop: string, sessionData: any): Promise<AppBridgeSession> {
    try {
      ...
      // Mettre à jour la session via ShopifyService
      await this.shopifyService.updateSession(shop, {
        ...
        onlineAccessInfo: updatedSession.onlineAccessInfo,
      });
  ...
  async getCurrentUser(shop: string): Promise<AppBridgeUser> {
    try {
      ...
      // Récupérer les informations utilisateur depuis Shopify API
      const userInfo = await this.shopifyService.getCurrentUser(shop, session.accessToken);
      
      if (!userInfo) {
        throw new Error('Impossible de récupérer les informations utilisateur depuis Shopify');
      }
      
      return {
        id: userInfo.id || `user_${shop}`,
        first_name: userInfo.first_name || '',
        last_name: userInfo.last_name || '',
        email: userInfo.email || '',
        account_owner: userInfo.account_owner || false,
        locale: userInfo.locale || 'en',
        permissions: userInfo.permissions || session.scope.split(','),
      };
    } catch (error) {
