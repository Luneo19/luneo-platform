...
export class AnalyticsAdvancedService {
  ...
  private async buildSegmentUserWhere(brandId: string, criteria: SegmentCriteria): Promise<Prisma.UserWhereInput> {
    ...
    const validRoles: PlatformRole[] = [PlatformRole.USER, PlatformRole.ADMIN];
    if (typeof role === 'string' && validRoles.includes(role as PlatformRole)) {
      baseWhere.role = role as PlatformRole;
    }
