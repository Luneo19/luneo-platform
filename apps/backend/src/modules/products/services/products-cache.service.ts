...
export class ProductsCacheService {
   ...
   */
  async getList(filters: Record<string, any>): Promise<any[] | null> {
    const key = this.getListKey(filters);
    return this.cache.get<any[]>(key, 'product');
  }

  /**
   * Met en cache une liste de produits
   */
  async setList(filters: Record<string, any>, products: any[]): Promise<void> {
    const key = this.getListKey(filters);
