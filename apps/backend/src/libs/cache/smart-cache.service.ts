...
export class SmartCacheService {
  ...
  /**
   * Set avec invalidation intelligente
   */
  async set<T>(
    key: string,
    type: string,
    ...
    try {
      const result = await this.redisService.set(cacheKey, cacheData, type, { 
        ttl,
        tags: options.tags 
      });

      // Invalider les caches liés si nécessaire
      if (options.tags) {
        await this.invalidateRelatedCaches(options.tags);
      }
   ...
   */
  async getSimple(key: string): Promise<any> {
    try {
   ...
   */
  async setSimple(key: string, data: any, ttl?: number): Promise<boolean> {
    try {
      const cacheData = {
        data,
        timestamp: Date.now(),
