...
export const PLAN_CONFIGS: Record<PlanTier, PlanConfig> = {
  [PlanTier.FREE]: {
    ...
    info: {
      ...
      features: ['1 agent IA', '100 conversations/mois', '1 base de connaissances', 'Widget chat'],
    },
    pricing: {
      monthlyPrice: 0,
      yearlyPrice: 0,
    ...
    quotas: [
      { metric: 'conversations', limit: 100, period: 'month', overage: 'block' },
      { metric: 'messages_ai', limit: 1_000, period: 'month', overage: 'block' },
  ...
  [PlanTier.PRO]: {
    ...
    quotas: [
      ...
      { metric: 'storage_gb', limit: 10, period: 'month', overage: 'charge', overageRate: 40 },
      { metric: 'api_calls', limit: 50_000, period: 'month', overage: 'charge', overageRate: 1 },
      { metric: 'team_members', limit: 5, period: 'month', overage: 'block' },
    ],
    agentLimits: {
  ...
  [PlanTier.BUSINESS]: {
    ...
    quotas: [
      { metric: 'conversations', limit: 15_000, period: 'month', overage: 'charge', overageRate: 5 },
      { metric: 'messages_ai', limit: 150_000, period: 'month', overage: 'charge', overageRate: 1 },
      ...
      { metric: 'team_members', limit: 25, period: 'month', overage: 'block' },
    ],
    agentLimits: {
...
export function normalizePlanTier(planId: string | null | undefined): PlanTier {
  ...
  const ALIASES: Record<string, PlanTier> = {
    free: PlanTier.FREE,
    trial: PlanTier.FREE,
    starter: PlanTier.PRO, // backward compat: existing DB rows with STARTER → PRO limits
    start: PlanTier.PRO,
    professional: PlanTier.PRO, // backward compat: old PROFESSIONAL → PRO
    pro: PlanTier.PRO,
