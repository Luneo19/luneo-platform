...
exports.ProductRulesService = void 0;
const common_1 = require("@nestjs/common");
let ProductRulesService = (() => {
    let _classDecorators = [(0, common_1.Injectable)()];
    let _classDescriptor;
    let _classExtraInitializers = [];
    let _classThis;
    var ProductRulesService = _classThis = class {
        constructor(prisma, cache) {
            this.prisma = prisma;
            this.cache = cache;
            this.logger = new common_1.Logger(ProductRulesService.name);
        }
        /**
        ...
        async updateProductRules(productId, rules) {
            try {
                ...
                return updatedProduct.rulesJson;
            }
            catch (error) {
         ...
         */
        async validateDesignOptions(context) {
            const errors = [];
            const warnings = [];
            try {
                // Valider chaque zone
                ...
                const compatibilityValidation = this.validateCompatibilityRules(context.rules.compatibilityRules || [], context.options);
                errors.push(...compatibilityValidation.errors);
                warnings.push(...compatibilityValidation.warnings);
                // Valider les contraintes globales
                const globalValidation = this.validateGlobalConstraints(context.rules.globalConstraints, context.options);
         ...
         */
        validateTextZone(zone, options, errors, warnings) {
         ...
         */
        validateImageZone(zone, options, errors, warnings) {
         ...
         */
        validateColorZone(zone, options, errors, warnings) {
            if (!options.color) {
                errors.push({
                    code: 'COLOR_REQUIRED',
         ...
         */
        validateSelectZone(zone, options, errors, warnings) {
            if (!options.value) {
                errors.push({
         ...
         */
        validateZoneConstraints(zone, options, errors, warnings) {
            if (!zone.constraints)
                return;
         ...
         */
        validateCompatibilityRules(rules, options) {
            const errors = [];
            const warnings = [];
         ...
         */
        validateGlobalConstraints(constraints, options) {
            const errors = [];
            const warnings = [];
            if (!constraints)
                return { errors, warnings };
         ...
         */
        evaluateRuleCondition(condition, options) {
         ...
         */
        hasOptionValue(options, optionPath, expectedValue) {
            const keys = optionPath.split('.');
            let current = options;
            for (const key of keys) {
                if (current && typeof current === 'object' && key in current) {
    ...
    __setFunctionName(_classThis, "ProductRulesService");
    (() => {
        const _metadata = typeof Symbol === "function" && Symbol.metadata ? Object.create(null) : void 0;
        __esDecorate(null, _classDescriptor = { value: _classThis }, _classDecorators, { kind: "class", name: _classThis.name, metadata: _metadata }, null, _classExtraInitializers);
        ProductRulesService = _classThis = _classDescriptor.value;
        if (_metadata) Object.defineProperty(_classThis, Symbol.metadata, { enumerable: true, configurable: true, writable: true, value: _metadata });
        __runInitializers(_classThis, _classExtraInitializers);
    })();
    return ProductRulesService = _classThis;
})();
exports.ProductRulesService = ProductRulesService;
