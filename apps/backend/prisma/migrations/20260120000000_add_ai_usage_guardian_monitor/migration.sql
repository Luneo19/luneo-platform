-- Migration: Add AI Usage Guardian & Monitor Models
-- Created: 2026-01-20
...
CREATE TABLE IF NOT EXISTS "AIAnalytics" (
    ...
    "totalCostCents" DECIMAL(10,2) NOT NULL DEFAULT 0,
...
-- AIUsageLog
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIUsageLog_brandId_fkey') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIUsageLog_agentId_fkey') THEN
...
-- AIQuota
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIQuota_brandId_fkey') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIQuota_userId_fkey') THEN
...
-- AIRateLimit
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIRateLimit_brandId_fkey') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIRateLimit_userId_fkey') THEN
...
-- AIAnalytics
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIAnalytics_brandId_fkey') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'AIAnalytics_agentId_fkey') THEN
