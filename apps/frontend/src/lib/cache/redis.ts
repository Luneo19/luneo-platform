...
export const cacheKeys = {
  ...
  orders: (userId: string, page: number, limit: number, status: string, search: string) =>
    `orders:${userId}:${page}:${limit}:${status}:${search}`,
};
...
export const cacheTTL = {
  ...
  ORDERS_LIST: 120, // 2 minutes pour les listes de commandes
