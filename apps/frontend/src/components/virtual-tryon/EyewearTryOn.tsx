...
export default function EyewearTryOn({
  modelUrl,
  lodLevels,
  scaleFactor = 1,
  defaultPosition,
  ...
  const initEngine = useCallback(async () => {
    ...
    try {
      ...
      const engine = new TryOnEngine(config, {
        ...
        onError: (err) => {
          logger.error('EyewearTryOn engine error', { error: err.message });
          onError?.(err);
        },
      });
      ...
      // Load eyewear 3D model
      if (modelUrl) {
        const renderer = new EyewearRenderer({
          modelUrl,
          lodLevels,
          scaleFactor,
          defaultPosition,
      ...
      const error = err instanceof Error ? err : new Error('Failed to init EyewearTryOn');
      logger.error('EyewearTryOn init failed', { error: error.message });
      onError?.(error);
    }
      ...
      <div className="absolute top-3 left-3 flex items-center gap-2">
        {isInitialized && (
          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
            ...
            {isTracking ? 'Visage détecté' : 'Regardez la caméra'}
          </span>
        )}
        {fps > 0 && (
