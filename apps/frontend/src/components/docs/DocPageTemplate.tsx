...
function DocPageTemplateContent({ 
  title, 
  description, 
  breadcrumbs, 
  children,
  ...
  const memoizedRelatedLinks = useMemo(() => relatedLinks, [relatedLinks]);
  return (
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div className="flex items-center text-sm text-gray-400 mb-6">
            {memoizedBreadcrumbs.map((crumb, index) => (
          ...
          {/* Related Links */}
          {memoizedRelatedLinks.length > 0 && (
            <div className="mt-12">
              <h3 className="text-2xl font-bold mb-6">Articles liés</h3>
              <div className="grid md:grid-cols-2 gap-4">
                {memoizedRelatedLinks.map((link, index) => (
...
const DocPageTemplateContentMemo = memo(DocPageTemplateContent);

export function DocPageTemplate(props: DocPageTemplateProps) {
  return (
    <ErrorBoundary componentName="DocPageTemplate">
      <DocPageTemplateContentMemo {...props} />
    </ErrorBoundary>
  );
}
