    ...
    <div ref={containerRef} className="relative w-full">
      <div className="relative flex items-center">
        ...
        <input
          ...
          className="dash-input w-full pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground bg-background border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-ring"
          aria-label={t('header.searchPlaceholder')}
        />
      </div>

      {showDropdown && (
        <div className="absolute top-full left-0 right-0 mt-1 max-h-[min(400px,70vh)] overflow-y-auto rounded-xl border border-border bg-popover shadow-2xl z-[100]">
          {loading ? (
            <div className="p-4 text-center text-muted-foreground text-sm">{t('common.loading')}</div>
          ) : allItems.length === 0 ? (
            <div className="p-4 text-center text-muted-foreground text-sm">{t('common.noResults')}</div>
          ) : (
            <div className="py-2">
              {allItems.slice(0, 15).map((item) => (
                ...
                >
                  {item.imageUrl ? (
                    ...
                    <div className="h-9 w-9 rounded bg-muted flex items-center justify-center text-muted-foreground text-xs font-medium">
                      {item.type === 'product' ? 'P' : item.type === 'design' ? 'D' : 'O'}
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="font-medium text-popover-foreground truncate">{item.title}</div>
                    {item.subtitle && (
