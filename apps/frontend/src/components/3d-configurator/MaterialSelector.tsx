...
function MaterialSelector({
  ...
  // Optimisé: useMemo pour filtrage
  ...
  // Optimisé: useMemo pour popular materials
  ...
  const handleToggleDetails = useCallback((materialId: string) => {
    setShowDetails(prev => prev === materialId ? null : materialId);
  }, []);
          ...
          <div className="grid grid-cols-2 gap-3">
            {popularMaterials.slice(0, 4).map((material) => (
              <MaterialCard
                ...
                onToggleDetails={() => handleToggleDetails(material.id)}
              />
            ))}
          </div>
        </div>
...
const MaterialSelectorMemo = memo(MaterialSelector);

export default function MaterialSelectorWithErrorBoundary(props: MaterialSelectorProps) {
  return (
    <ErrorBoundary componentName="MaterialSelector">
      <MaterialSelectorMemo {...props} />
    </ErrorBoundary>
  );
}
