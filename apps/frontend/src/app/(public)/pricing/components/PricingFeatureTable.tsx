...
function formatCell(value: boolean | string) {
  if (typeof value === 'boolean') {
    return value ? <Check className="mx-auto h-5 w-5 text-green-400" /> : <X className="mx-auto h-5 w-5 text-slate-400" />;
  }
    ...
    <div className="overflow-x-auto">
      {(displayTitle || displaySubtitle) && (
        <div className="mb-12 text-center">
          ...
          {displaySubtitle && <p className="mt-4 text-lg text-slate-300">{displaySubtitle}</p>}
        </div>
      )}
