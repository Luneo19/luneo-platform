...
import { CreditPacksSection } from '@/components/credits/CreditPacksSection';
...
// Fonction pour obtenir l'icône marketing (appelée côté client uniquement)
...
// MARKETING_PRESENTATION sans JSX (utilise iconType au lieu de icon)
const MARKETING_PRESENTATION: Record<
  PlanTier,
  {
    description: string;
    iconType: 'sparkles' | 'zap' | 'rocket' | 'building2';
    gradient: string;
    cta: string;
    planId?: string;
    href?: string;
...
// Fonction helper pour obtenir la présentation complète avec icône (côté client uniquement)
...
// Fonction helper pour créer les icônes (appelée côté client uniquement)
...
const DEFAULT_PLANS_DATA = [
  {
    ...
    yearlyDiscount: 0,
    features: [
      { name: '10 designs/mois', included: true },
      { name: 'Customizer 2D', included: true },
      { name: 'Support standard', included: true },
    ],
    quotas: [
      { label: 'Designs', value: '10', period: '/mois' },
      { label: 'Stockage', value: '1 GB', period: '' },
    ],
  },
  {
    ...
    quotas: [
      { label: 'Designs', value: '250', period: '/mois' },
      { label: 'Stockage', value: '50 GB', period: '' },
    ],
  },
  {
    ...
    quotas: [
      { label: 'Designs', value: '1000', period: '/mois' },
      { label: 'Stockage', value: '100 GB', period: '' },
    ],
  },
  {
    ...
    quotas: [
      { label: 'Designs', value: 'Illimité', period: '' },
      { label: 'Stockage', value: 'Illimité', period: '' },
    ],
  },
];
...
const buildPricingPlans = (): PricingPlanCard[] => {
  ...
  if (!PLAN_CATALOG || !PLAN_DEFINITIONS || !PLAN_CATALOG.availableTiers || PLAN_CATALOG.availableTiers.length === 0) {
    // Convertir les plans par défaut en PricingPlanCard avec icônes
  ...
  try {
    ...
    return PLAN_CATALOG.availableTiers.map((tier) => {
      ...
      const monthlyPrice = (definition.basePriceCents || 0) / 100;
      ...
      return {
        ...
        gradient: marketing.gradient || 'from-gray-500 to-gray-600',
        cta: marketing.cta || 'Commencer',
    ...
    }).filter((plan): plan is PricingPlanCard => plan !== null && plan !== undefined);
  } catch (error) {
...
// Ne pas appeler buildPricingPlans() au niveau du module - le faire dans le composant
...
function PricingPageContent() {
  ...
  // Construire les plans statiques (côté client uniquement)
  const staticPlans = useMemo(() => buildPricingPlans(), []);
  ...
  const mergedPlans = useMemo(() => {
    ...
    return dynamicPlans.map((dynamicPlan: any) => {
      // Trouver le plan statique correspondant
      const planId = dynamicPlan.id || dynamicPlan.name?.toLowerCase() || '';
      const staticPlan = staticPlans.find(
        (p) => p.tier === planId || p.name.toLowerCase() === planId
      );
      ...
      if (staticPlan) {
        ...
        return {
          ...
          features: (Array.isArray(dynamicPlan.features) && dynamicPlan.features.length > 0)
            ? dynamicPlan.features.map((f: any) => ({ name: f.name || '', included: f.included ?? true }))
            : staticPlan.features,
        };
      }
      ...
      return {
        tier: (dynamicPlan.id || 'starter') as PlanTier,
        name: dynamicPlan.name || 'Plan',
        icon: <Sparkles className="w-6 h-6" />,
        description: dynamicPlan.description || '',
        gradient: 'from-gray-500 to-gray-600',
        cta: 'Commencer',
        planId: dynamicPlan.id || 'starter',
        priceMonthly: dynamicPlan.price?.monthly ?? 0,
        yearlyPrice: dynamicPlan.price?.yearly ?? 0,
        yearlyDiscount: 0.2,
        features: Array.isArray(dynamicPlan.features) 
          ? dynamicPlan.features.map((f: any) => ({ name: f.name || '', included: f.included ?? true }))
          : [],
        quotas: [],
        popular: dynamicPlan.popular || false,
      };
    });
  }, [dynamicPlans]);
  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-lg text-gray-300 max-w-2xl mx-auto">
              Complétez votre abonnement avec des packs de crédits supplémentaires.
              <br />
              <span className="text-purple-400 font-semibold">Parfait pour les pics d'activité ou les projets ponctuels.</span>
            </p>
          </motion.div>
      ...
      {/* CREDIT PACKS SECTION */}
      {/* ============================================ */}
      <section className="py-16 sm:py-20 bg-gradient-to-br from-gray-900 via-purple-950/20 to-blue-950/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <CreditPacksSection />
        </div>
      </section>
...
const MemoizedPricingPageContent = memo(PricingPageContent);

export default function PricingPage() {
  return (
    <ErrorBoundary level="page" componentName="PricingPage">
      <MemoizedPricingPageContent />
    </ErrorBoundary>
  );
}
