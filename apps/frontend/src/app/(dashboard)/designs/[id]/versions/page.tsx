 ...
 * TODO-037: UI complète pour versioning (200+ lignes)
 */
export default function DesignVersionsPage() {
  ...
  const loadDesign = async () => {
    try {
      ...
      logger.error('Error loading design', {
        error,
        designId,
  ...
  const loadVersions = async () => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || 'Erreur lors du chargement des versions');
      }

      setVersions(result.data.versions || []);
    } catch (err: any) {
      logger.error('Error loading versions', {
        error: err,
        designId,
        message: err.message || 'Unknown error',
      });
      setError(err.message || 'Impossible de charger les versions');
      toast({
        title: 'Erreur',
        description: err.message || 'Impossible de charger les versions',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
