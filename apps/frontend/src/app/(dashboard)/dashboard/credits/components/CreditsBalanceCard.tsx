    ...
    <Card className="bg-gradient-to-br from-cyan-50 to-blue-50 border-cyan-500/20">
      <CardContent className="p-8">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-gray-600 mb-2">{t('credits.availableCredits')}</p>
            <div className="flex items-baseline gap-3">
              ...
              >
                {stats.currentBalance > 100 ? t('credits.goodLevel') : stats.currentBalance > 50 ? t('credits.mediumLevel') : t('credits.lowLevel')}
              </Badge>
            </div>
            <div className="flex items-center gap-4 mt-4">
              <div>
                <p className="text-sm text-gray-600">{t('credits.estimatedValue')}</p>
              ...
              <div>
                <p className="text-sm text-gray-600">{t('credits.usageRateLabel')}</p>
          ...
          <div className="text-right">
            {stats.currentBalance < 100 && (
              <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-lg mb-4">
                ...
                <p className="text-sm text-red-400 font-medium mb-1">{t('credits.lowBalance')}</p>
                <p className="text-xs text-gray-600 mb-3">{t('credits.rechargeHint')}</p>
            ...
            {autoRefillEnabled && (
              <div className="p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="text-sm text-green-400 font-medium">{t('credits.autoRefillActive')}</p>
                </div>
                <p className="text-xs text-gray-600">{t('credits.autoRefillAt', { count: autoRefillThreshold })}</p>
              </div>
            )}
          </div>
        </div>
