        ...
        <div className="flex">
          ...
          <CardContent className="p-4 flex-1 flex items-center justify-between gap-4">
            <div className="min-w-0 flex-1">
              <p className="text-sm text-white line-clamp-1">{model.name || model.prompt}</p>
              <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
                <Badge variant="outline" className="border-slate-600 text-xs">{model.category}</Badge>
                <span>{formatRelativeTimeFn(ts)}</span>
              </div>
            </div>
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button variant="ghost" size="icon" onClick={() => onPreview(model)} className="text-white hover:bg-white/20 h-8 w-8">
                <Eye className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => onToggleFavorite(model.id)} className={cn('text-white hover:bg-white/20 h-8 w-8', model.isFavorite && 'text-pink-400')}>
                {model.isFavorite ? <Heart className="w-4 h-4 fill-current" /> : <Heart className="w-4 h-4" />}
              </Button>
              <Button variant="ghost" size="icon" onClick={() => onViewDetails(model)} className="text-white hover:bg-white/20 h-8 w-8">
                <Info className="w-4 h-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={onExport} className="text-white hover:bg-white/20 h-8 w-8">
