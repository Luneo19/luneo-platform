...
export async function POST(
  ...
  return ApiResponseBuilder.handle(async () => {
    ...
    // Validation
    const validated = PODSubmitSchema.parse(body);
    ...
    const { podMappingService } = await import('@/lib/services/PODMappingService');
    ...
    switch (provider.toLowerCase()) {
      case 'printful':
        podOrderId = await submitToPrintful(order, productionFiles, validated.autoFulfill);
        trackingUrl = validated.autoFulfill ? `https://www.printful.com/track/${podOrderId}` : undefined;
        break;

      case 'printify':
        podOrderId = await submitToPrintify(order, productionFiles, validated.autoFulfill);
        trackingUrl = validated.autoFulfill ? `https://printify.com/track/${podOrderId}` : undefined;
        break;

      case 'gelato':
        podOrderId = await submitToGelato(order, productionFiles, validated.autoFulfill);
        trackingUrl = validated.autoFulfill ? `https://gelato.com/track/${podOrderId}` : undefined;
        break;

      default:
        return ApiResponseBuilder.badRequest(`Unsupported provider: ${provider}`);
    }
  ...
  }, `/api/pod/${params.provider}/submit`, 'POST');
}
...
async function submitToPrintful(
  order: any,
  productionFiles: Array<{ itemId: string; fileUrl: string; quantity: number; variantId?: string | number; productId?: string | number; productUid?: string; productName?: string }>,
  autoFulfill: boolean
): Promise<string> {
  try {
    ...
    const printfulOrder = {
      ...
      items: productionFiles.map((file) => {
        ...
        const variantId = file.variantId 
          ? (typeof file.variantId === 'string' ? parseInt(file.variantId, 10) : file.variantId)
    ...
    logger.error('Error submitting to Printful', { error, orderId: order.id });
    // Fallback: generate mock ID
    return `printful_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  }
}
...
async function submitToPrintify(
  ...
  try {
    ...
    const printifyOrder = {
      line_items: productionFiles.map((file) => {
        ...
        const productId = file.productId 
          ? (typeof file.productId === 'string' ? parseInt(file.productId, 10) : file.productId)
          : 1;
        const variantId = file.variantId 
          ? (typeof file.variantId === 'string' ? parseInt(file.variantId, 10) : file.variantId)
          : 1;
    ...
    logger.error('Error submitting to Printify', { error, orderId: order.id });
    return `printify_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  }
}
  ...
  try {
    ...
    const gelatoOrder = {
      items: productionFiles.map((file) => {
        ...
        const productUid = file.productUid || file.productId?.toString() || `product_${order.productId}`;
    ...
    logger.error('Error submitting to Gelato', { error, orderId: order.id });
    return `gelato_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
  }
}
