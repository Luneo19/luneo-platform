...
export async function GET(request: NextRequest) {
  return ApiResponseBuilder.handle(async () => {
    ...
    const response = await fetch(`${backendUrl}/api/analytics/funnel?${params.toString()}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      logger.error('Backend error for funnel', { status: response.status, error: errorText });
