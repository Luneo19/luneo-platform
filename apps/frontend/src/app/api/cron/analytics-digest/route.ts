/**
 * GET/POST /api/cron/analytics-digest
 * Scheduled cron job to generate analytics digest
 * Forward vers backend NestJS: GET/POST /cron/analytics-digest
 */
...
async function handleCronJob(request: NextRequest, method: 'GET' | 'POST') {
  ...
  if (cronSecret && authHeader !== `Bearer ${cronSecret}`) {
    logger.warn('Unauthorized cron job request', {
      endpoint: '/api/cron/analytics-digest',
      hasAuth: !!authHeader,
    });
    return new Response(
  ...
  try {
    ...
    const url = `${apiUrl}/cron/analytics-digest`;

    logger.info('Forwarding analytics-digest cron job to backend', { url });
    ...
    logger.error('Failed to forward analytics-digest cron job', {
