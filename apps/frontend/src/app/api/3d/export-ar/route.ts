...
function generateARLinks(modelUrl: string, platform: string, title: string): any {
  ...
  if (platform === 'ios') {
    ...
    // Note: Using optimized image URL for better performance
    const arIconUrl = '/ar-icon-ios.png'; // Will be optimized by Next.js Image Optimization
    links.quickLook = `<a rel="ar" href="${modelUrl}">
      <img src="${arIconUrl}" alt="View in AR on iOS" width="120" height="120" loading="lazy" />
    </a>`;
