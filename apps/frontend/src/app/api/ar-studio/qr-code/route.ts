...
export async function POST(request: NextRequest) {
  return ApiResponseBuilder.handle(async () => {
    ...
    const response = await fetch(`${backendUrl}/api/ar-studio/models/${modelId}/qr-code`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      logger.error('Backend error for AR QR code', { status: response.status, error: errorText });
