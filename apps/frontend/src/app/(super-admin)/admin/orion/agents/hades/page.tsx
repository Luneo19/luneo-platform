...
import { ArrowLeft, DollarSign, Loader2, CreditCard, TrendingUp, ArrowUpCircle, Sparkles } from 'lucide-react';
...
type RevenueData = {
  ...
  ltv?: number;
  upsellTriggers?: Record<string, unknown>;
} | null;
...
export default function OrionHadesPage() {
  const { agent, isLoading: agentLoading, error: agentError, refresh } = useAgentDetail('REVENUE');
  const { data: domainData } = useAgentDomainData<RevenueData>('/api/admin/orion/revenue');
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-zinc-400">Revenue Optimizer</p>
      ...
      {!isError && (
        <>
            ...
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <CreditCard className="h-4 w-4" /> ARPU
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.arpu)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <TrendingUp className="h-4 w-4" /> Expansion Revenue
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.expansionRevenue)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <ArrowUpCircle className="h-4 w-4" /> Upsell Conversion
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.upsellConversion)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <Sparkles className="h-4 w-4" /> LTV
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.ltv)}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="bg-zinc-800/50 border-zinc-700">
            <CardHeader>
              <CardTitle className="text-white">Upsell triggers</CardTitle>
              <p className="text-sm text-zinc-400">Current configuration (read-only)</p>
            </CardHeader>
            <CardContent className="space-y-2">
              {domainData?.upsellTriggers && Object.keys(domainData.upsellTriggers).length > 0 ? (
                <dl className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
                  {Object.entries(domainData.upsellTriggers).map(([key, value]) => (
                ...
                <p className="text-zinc-500 text-sm">— No upsell triggers configured</p>
              )}
            </CardContent>
          </Card>
