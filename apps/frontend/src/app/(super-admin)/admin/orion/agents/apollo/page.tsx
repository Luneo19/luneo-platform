...
import { ArrowLeft, Rocket, Loader2, UserPlus, Percent, DollarSign, BarChart3 } from 'lucide-react';
...
export default function OrionApolloPage() {
  const { agent, isLoading: agentLoading, error: agentError, refresh } = useAgentDetail('ACQUISITION');
  const { data: domainData, isLoading: domainLoading } = useAgentDomainData<LeadsData>('/api/admin/orion/revenue/leads');
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-zinc-400">Acquisition Intelligence</p>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-zinc-800/50 border-zinc-700">
              ...
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.newSignups)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.conversionRate)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <DollarSign className="h-4 w-4" /> CAC
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.cac)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <BarChart3 className="h-4 w-4" /> Attribution Score
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.attributionScore)}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="bg-zinc-800/50 border-zinc-700">
            <CardHeader>
              <CardTitle className="text-white">Lead scoring weights</CardTitle>
              <p className="text-sm text-zinc-400">Current configuration (read-only)</p>
            </CardHeader>
            <CardContent className="space-y-2">
              {domainData?.leadScoringWeights && Object.keys(domainData.leadScoringWeights).length > 0 ? (
                <dl className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-sm">
                  {Object.entries(domainData.leadScoringWeights).map(([key, value]) => (
                ...
                <p className="text-zinc-500 text-sm">— No lead scoring weights configured</p>
              )}
            </CardContent>
          </Card>
