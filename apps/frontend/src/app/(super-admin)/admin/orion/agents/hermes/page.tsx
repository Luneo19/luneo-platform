...
import { ArrowLeft, Mail, Loader2, Send, MailOpen, MousePointer, CheckCircle } from 'lucide-react';
...
export default function OrionHermesPage() {
  const { agent, isLoading: agentLoading, error: agentError, refresh } = useAgentDetail('COMMUNICATION');
  const { data: domainData, isLoading: domainLoading } = useAgentDomainData<CommsStats>('/api/admin/orion/communications/stats');
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-zinc-400">Communication Hub</p>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <Card className="bg-zinc-800/50 border-zinc-700">
              ...
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.emailsSent)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <MailOpen className="h-4 w-4" /> Open Rate
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.openRate)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.clickRate)}</p>
              </CardContent>
            </Card>
            <Card className="bg-zinc-800/50 border-zinc-700">
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-zinc-400 flex items-center gap-2">
                  <CheckCircle className="h-4 w-4" /> Delivery Rate
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-2xl font-bold text-white">{formatVal(domainData?.deliveryRate)}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card className="bg-zinc-800/50 border-zinc-700">
            <CardHeader>
              <CardTitle className="text-white">Email channel preferences</CardTitle>
            ...
            <CardContent className="space-y-2">
              <dl className="grid grid-cols-2 gap-2 text-sm">
                <div className="flex justify-between gap-2 py-1 border-b border-zinc-700/50">
                  <dt className="text-zinc-400">Email channel enabled</dt>
                  <dd className="text-white font-medium">
                    {domainData?.emailChannelEnabled != null ? (domainData.emailChannelEnabled ? 'Yes' : 'No') : '—'}
                  </dd>
                </div>
                <div className="flex justify-between gap-2 py-1 border-b border-zinc-700/50">
                  <dt className="text-zinc-400">Default provider</dt>
                  <dd className="text-white font-medium">{formatVal(domainData?.defaultProvider)}</dd>
                </div>
              </dl>
            </CardContent>
          </Card>
