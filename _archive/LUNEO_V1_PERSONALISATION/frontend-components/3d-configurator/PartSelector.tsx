...
function PartSelector({
  ...
  // Optimisé: useMemo pour activeCategoryData
  const activeCategoryData = useMemo(() => {
    return categories.find(c => c.id === activeCategory);
  }, [categories, activeCategory]);

  // Optimisé: useMemo pour filteredParts
  const filteredParts = useMemo(() => {
    if (!activeCategoryData) return [];
    
    return activeCategoryData.parts.filter((part) => {
  ...
  // Optimisé: useCallback pour isPartSelected
  const isPartSelected = useCallback((categoryId: string, partId: string): boolean => {
    const categoryParts = selectedParts.get(categoryId) || [];
    return categoryParts.includes(partId);
  }, [selectedParts]);

  // Optimisé: useCallback pour handlePartClick
  const handlePartClick = useCallback((categoryId: string, partId: string, multipleSelection: boolean) => {
  ...
  }, [isPartSelected, onPartSelect, onPartRemove, selectedParts]);

  // Optimisé: useMemo pour calculateTotalPrice
  ...
  // Optimisé: useMemo pour selectedCount
  const selectedCountByCategory = useMemo(() => {
    const counts = new Map<string, number>();
    categories.forEach(category => {
      counts.set(category.id, (selectedParts.get(category.id) || []).length);
    });
    return counts;
  }, [categories, selectedParts]);

  const handleToggleDetails = useCallback((partId: string) => {
    setShowDetails(prev => prev === partId ? null : partId);
  }, []);
...
const PartSelectorMemo = memo(PartSelector);

export default function PartSelectorWithErrorBoundary(props: PartSelectorProps) {
  return (
    <ErrorBoundary componentName="PartSelector">
      <PartSelectorMemo {...props} />
    </ErrorBoundary>
  );
}
