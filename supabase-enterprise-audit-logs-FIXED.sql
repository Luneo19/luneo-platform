-- ============================================
-- ENTERPRISE AUDIT LOGS SYSTEM (VERSION CORRIGÉE)
-- Traçabilité complète pour RGPD & Enterprise
-- ⚠️ ATTENTION: Exécuter APRÈS supabase-orders-system.sql
-- ============================================
...
-- ⚠️ SEULEMENT SI LES TABLES EXISTENT
-- ============================================

-- Logger les changements sur les commandes (SI TABLE EXISTE)
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Créer la fonction pour logger les changements
    ...
    RAISE NOTICE '✅ Trigger audit pour orders créé';
  ELSE
    RAISE NOTICE '⚠️ Table orders non trouvée - trigger non créé (normal si orders_system pas encore exécuté)';
  END IF;
END $$;

-- Logger les changements sur les designs (SI TABLE EXISTE)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    ...
    RAISE NOTICE '✅ Trigger audit pour designs créé';
  ELSE
    RAISE NOTICE '⚠️ Table designs non trouvée - trigger non créé';
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '⚡ Triggers automatiques configurés (si tables existent)';
