...
-- Version: CORRIGÉE (colonnes existantes uniquement)
-- =====================================================
...
-- Index de recherche full-text (adapté au schéma actuel)
CREATE INDEX IF NOT EXISTS idx_cliparts_search 
ON public.cliparts USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '') || ' ' || array_to_string(tags, ' ')));
...
-- 8. INTEGRATIONS - Optimiser récupération intégrations (si table existe)
-- =====================================================
...
-- 9. NOTIFICATIONS - Optimiser récupération notifications (si table existe)
-- =====================================================
...
-- 10. AUDIT_LOGS - Optimiser recherche logs (si table existe)
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '📊 25+ indexes créés pour améliorer les performances.';
END $$;
